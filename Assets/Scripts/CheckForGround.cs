...
public class CheckForGround : MonoBehaviour
{
    ...
    public bool IsGrounded(float extraDetectionRayLength = 0.0f)
    {
        ...
        if (Physics.Raycast(midRay, out RaycastHit midHitInfo, GroundRayDistance + extraDetectionRayLength))
        {
        ...
        if (Physics.Raycast(leftRay, out RaycastHit leftHitInfo, GroundRayDistance + extraDetectionRayLength))
        {
        ...
        if (Physics.Raycast(rightRay, out RaycastHit rightHitInfo, GroundRayDistance + extraDetectionRayLength))
        {
        ...
        if (Physics.Raycast(frontRay, out RaycastHit frontHitInfo, GroundRayDistance + extraDetectionRayLength))
        {
        ...
        if (Physics.Raycast(backRay, out RaycastHit backHitInfo, GroundRayDistance + extraDetectionRayLength))
        {
